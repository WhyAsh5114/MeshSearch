...
`);
  ...
  const isLocalRpc = (url: string) =>
    !url || url.includes('localhost') || url.includes('127.0.0.1');

  // If the existing RPC is local, treat stored addresses as stale (Hardhat defaults)
  const existingRpc = mcpEnv['RPC_URL'] || '';
  const staleLocal  = isLocalRpc(existingRpc);

  const addr = (val: string | undefined, fallback: string) =>
    (!val || val === zero || staleLocal) ? fallback : val;

  let rpcUrl          = staleLocal
    ? 'https://base-sepolia.g.alchemy.com/v2/TsD5q7yvfBmn6byKNh4xm'
    : (existingRpc || 'https://base-sepolia.g.alchemy.com/v2/TsD5q7yvfBmn6byKNh4xm');
  let nodeReg         = addr(mcpEnv['NODE_REGISTRY_ADDRESS'],      '0xB9441DE5339304B1A071cd3aa30F5A1792a14f62');
  let nullifierReg    = addr(mcpEnv['NULLIFIER_REGISTRY_ADDRESS'],  '0xf7929A2f91785A01cE04C044aa34839D5657989e');
  let paymentSplitter = addr(mcpEnv['PAYMENT_SPLITTER_ADDRESS'],    '0xAFB6F1C87A45Bbf3ef42624EE3db66bD558dC17B');
  let accessControl   = addr(mcpEnv['ACCESS_CONTROL_ADDRESS'],      '0xc5c52C190BC3B8bdf7DC21F363EA5023beb31143');
  ...
  // ── Step 5c: BitGo wallet config ────────────────────────────────────────────
  console.log(c.step(5, 'BitGo wallet infrastructure (optional)'));

  const existingBitGo = mcpEnv['BITGO_ACCESS_TOKEN'] || '';
  let bitgoEnabled = !!existingBitGo;
  let bitgoAccessToken   = mcpEnv['BITGO_ACCESS_TOKEN'] || '';
  let bitgoWalletId      = mcpEnv['BITGO_WALLET_ID'] || '';
  let bitgoPassphrase    = mcpEnv['BITGO_WALLET_PASSPHRASE'] || '';
  let bitgoEnv           = mcpEnv['BITGO_ENV'] || 'test';
  let bitgoCoin          = mcpEnv['BITGO_COIN'] || 'hteth';
  let bitgoExpressUrl    = mcpEnv['BITGO_EXPRESS_URL'] || 'http://localhost:3080';
  let bitgoWebhookSecret = mcpEnv['BITGO_WEBHOOK_SECRET'] || '';
  let relay1WalletId     = mcpEnv['RELAY1_BITGO_WALLET_ID'] || '';
  let relay2WalletId     = mcpEnv['RELAY2_BITGO_WALLET_ID'] || '';
  let relay3WalletId     = mcpEnv['RELAY3_BITGO_WALLET_ID'] || '';

  if (bitgoEnabled) {
    console.log(c.ok(`BitGo already configured — wallet=${bitgoWalletId.slice(0, 10)}…`));
    const reconfig = await askYN('Reconfigure BitGo settings?', false);
    if (reconfig) bitgoEnabled = false; // will re-prompt below
  }
  if (!bitgoEnabled) {
    bitgoEnabled = await askYN('Enable BitGo wallet infrastructure (stealth-address payments)?', false);
    if (bitgoEnabled) {
      bitgoAccessToken = (await ask('BitGo access token:')).trim();
      if (!bitgoAccessToken) {
        console.log(c.warn('No access token — BitGo disabled'));
        bitgoEnabled = false;
      } else {
        bitgoWalletId   = (await ask('Treasury wallet ID:')).trim();
        bitgoPassphrase = (await ask('Wallet passphrase:')).trim();
        const envChoice = await ask(`BitGo env: (1) test  (2) prod  [1]:`);
        if (envChoice.trim() === '2') bitgoEnv = 'prod';
        bitgoCoin = (await ask(`Coin [${bitgoCoin}]:`)).trim() || bitgoCoin;
        bitgoExpressUrl = (await ask(`BitGo Express URL [${bitgoExpressUrl}]:`)).trim() || bitgoExpressUrl;
        bitgoWebhookSecret = (await ask('Webhook secret (leave blank to set later):')).trim();
        relay1WalletId = (await ask('Relay 1 wallet ID:')).trim();
        relay2WalletId = (await ask('Relay 2 wallet ID:')).trim();
        relay3WalletId = (await ask('Relay 3 wallet ID:')).trim();
        console.log(c.ok(`BitGo: ${bitgoEnv}/${bitgoCoin} treasury=${bitgoWalletId.slice(0, 10)}…`));
      }
    }
  }
  ...
  if (bitgoEnabled) {
    mcpEnvVars['BITGO_ACCESS_TOKEN'] = bitgoAccessToken;
    mcpEnvVars['BITGO_WALLET_ID'] = bitgoWalletId;
    mcpEnvVars['BITGO_WALLET_PASSPHRASE'] = bitgoPassphrase;
    mcpEnvVars['BITGO_ENV'] = bitgoEnv;
    mcpEnvVars['BITGO_COIN'] = bitgoCoin;
    mcpEnvVars['BITGO_EXPRESS_URL'] = bitgoExpressUrl;
    mcpEnvVars['BITGO_WEBHOOK_SECRET'] = bitgoWebhookSecret;
    mcpEnvVars['RELAY1_BITGO_WALLET_ID'] = relay1WalletId;
    mcpEnvVars['RELAY2_BITGO_WALLET_ID'] = relay2WalletId;
    mcpEnvVars['RELAY3_BITGO_WALLET_ID'] = relay3WalletId;
  }
  ...
  spawnService('mcp-server', ports.mcp, 'apps/mcp-server', {
    ...
    ...(bitgoEnabled ? {
      BITGO_ACCESS_TOKEN: bitgoAccessToken,
      BITGO_WALLET_ID: bitgoWalletId,
      BITGO_WALLET_PASSPHRASE: bitgoPassphrase,
      BITGO_ENV: bitgoEnv,
      BITGO_COIN: bitgoCoin,
      BITGO_EXPRESS_URL: bitgoExpressUrl,
      BITGO_WEBHOOK_SECRET: bitgoWebhookSecret,
      RELAY1_BITGO_WALLET_ID: relay1WalletId,
      RELAY2_BITGO_WALLET_ID: relay2WalletId,
      RELAY3_BITGO_WALLET_ID: relay3WalletId,
    } : {}),
  });
...
${bitgoEnabled ? `  BitGo Express → ${bitgoExpressUrl}` : ''}

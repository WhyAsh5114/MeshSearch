...
function outcomeVariant(outcome: McpTraceEntry["outcome"]): "default" | "secondary" | "destructive" | "outline" {
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section className="mx-auto flex w-full max-w-6xl flex-col gap-4 px-4 py-6 md:px-6">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Card size="sm">
            <CardHeader>
              <CardTitle className="text-xs text-muted-foreground">Payment Required</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-2xl font-semibold text-amber-400">{stats.paymentRequired}</p>
            </CardContent>
          </Card>
          <Card size="sm">
            <CardHeader>
        ...
        <Card size="sm">
          <CardContent className="flex flex-wrap items-center justify-between gap-3 pt-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="destructive" size="sm" onClick={() => void onClear()} className="gap-1.5">
                ...
                <div className="space-y-1 p-2">
                  ...
                  {!loading && traces.length === 0 && (
                    <div className="rounded-lg border border-dashed border-border bg-muted/20 px-3 py-8 text-center text-sm text-muted-foreground">
                      No MCP requests yet. Run a query in chat to populate this inspector.
                    </div>
                  )}
                    ...
                    return (
                      ...
                      >
                        <div className="mb-1 flex items-center justify-between gap-2">
                          <span className="truncate font-mono text-xs text-primary">{trace.method}</span>
                          <Badge variant={outcomeVariant(trace.outcome)} className="text-[10px] uppercase">
                            {trace.outcome}
                          </Badge>
                        </div>
                        ...
                        {trace.query && (
                          <p className="mt-2 truncate text-xs text-foreground/90">{trace.query}</p>
                        )}
                      </button>
                    );
                  })}
            ...
            <CardContent className="p-4">
              ...
              {selected && (
                <Tabs defaultValue="overview" className="w-full">
                  <TabsList variant="line" className="mb-2 grid w-full grid-cols-4">
                  ...
                  <TabsContent value="overview" className="space-y-3">
                    <div className="grid gap-2 sm:grid-cols-2">
                      ...
                      <InfoRow label="Status" value={String(selected.statusCode ?? "-")} />
                      <InfoRow label="Latency" value={formatLatency(selected.latencyMs)} />
                      <InfoRow label="Tool" value={selected.toolName || "-"} />
                      <InfoRow label="Request Id" value={String(selected.requestId ?? "-")} />
                    </div>

                    <Accordion defaultValue={["routing"]} className="rounded-lg border border-border/60 px-3">
                      <AccordionItem value="routing" className="border-b-0">
                        ...
                        <AccordionContent>
                          <div className="space-y-2 pb-1 text-xs">
                            <InfoRow label="Session" value={selected.sessionId || "-"} />
                            <InfoRow
                              label="Payment Signature"
                              value={selected.hasPaymentSignature ? "present" : "absent"}
                            />
                            <InfoRow label="Query" value={selected.query || "-"} />
                            {selected.txHash && <InfoRow label="Tx Hash" value={selected.txHash} />}
                          </div>
                        </AccordionContent>
                      </AccordionItem>
                    </Accordion>
                    ...
                    {selected.resultPreview && (
                      <div className="rounded-lg border border-border/60 bg-muted/30 p-3">
                        ...
                        <p className="text-xs leading-relaxed text-foreground/90">{selected.resultPreview}</p>
                      </div>
                    )}
                  </TabsContent>
                  ...
                  <TabsContent value="metadata" className="space-y-3">
                    {selected.metadata ? (
                      <div className="grid gap-2">
                        <InfoRow label="Results" value={selected.metadata.totalResults || "-"} />
                        <InfoRow label="Search Time" value={selected.metadata.searchTime || "-"} />
                        <InfoRow label="Result Hash" value={selected.metadata.resultHash || "-"} />
                        <InfoRow label="Routing" value={selected.metadata.routing || "-"} />
                      </div>
                    ) : (
                  ...
                  <TabsContent value="payload" className="space-y-3">
                    <JsonBlock title="Request Headers" value={selected.requestHeaders} />
                    <JsonBlock title="Request Payload" value={selected.requestPayload} />
                  </TabsContent>

                  <TabsContent value="response" className="space-y-3">
                    <JsonBlock title="Response Summary" value={selected.responseSummary} />
...
function JsonBlock({ title, value }: { title: string; value: unknown }) {
  return (
    <div className="rounded-lg border border-border/60 bg-muted/20 p-3">
      <p className="mb-1 text-[11px] uppercase tracking-[0.08em] text-muted-foreground">{title}</p>
